#!/usr/bin/env python3
"""Deterministically link canonical My Business Zillow closings to FUB people.

FUB is read-only. Command Center is updated only with --apply. Auto-linking requires
an exact Zillow Premier Agent contact ID or a unique, corroborated FUB deal match.
"""
...
def is_canonical_zillow_closing(
    ...
    if report_year == current_year and transaction.get("archived") is True:
        return False
    return "zillow" in str(transaction.get("lead_source") or "").lower()
...
def _person_score(transaction: dict, zillow_row: dict | None, person: dict) -> tuple[int, set[str]]:
    ...
    if "zillow" in source_blob:
        score += 15
        evidence.add("zillow_source")
...
def choose_fub_person(transaction: dict, zillow_row: dict | None, people: Iterable[dict]) -> dict | None:
    ...
    for person in people:
        ...
        if person_id is None or str(person_id) in seen:
            continue
        seen.add(str(person_id))
        score, evidence = _person_score(transaction, zillow_row, person)
        candidates.append((score, str(person_id), evidence))
    candidates.sort(key=lambda item: item[0], reverse=True)
    if not candidates:
        return None
    explicit_candidates = [
    ...
    if explicit_candidates:
        if len(explicit_candidates) != 1:
            return None
        _, person_id, _ = explicit_candidates[0]
        return {"person_id": person_id, "method": "exact_pa_contact_id", "confidence": "explicit"}
    ...
    if score >= 150 and score - second_score >= 30 and has_property_evidence and "name" in evidence:
        return {"person_id": person_id, "method": "corroborated_fub_deal", "confidence": "high"}
    return None
...
def run(*, apply: bool, full_scan: bool, since_days: int, report_year: int) -> dict:
    ...
    app = create_app()
    summary = {"year": report_year, "eligible": 0, "linked": 0, "explicit": 0, "high": 0, "unresolved": 0, "apply": apply}
    with app.app_context():
        has_unlinked = Transaction.query.filter(
            ...
            Transaction.lead_source.ilike("%zillow%"),
        ).first() is not None
    if not has_unlinked:
        return summary
    ...
    people = {}
    if full_scan:
        for source in ("Zillow", "Zillow Preferred", "ZillowPremier"):
            for person in _iter_people(http, {"source": source}):
                people[str(person["id"])] = person
    else:
        updated_after = (datetime.utcnow() - timedelta(days=since_days)).strftime("%Y-%m-%dT%H:%M:%SZ")
        for person in _iter_people(http, {"updatedAfter": updated_after}):
            people[str(person["id"])] = person
    ...
    with app.app_context():
        ...
        for transaction in transactions:
            ...
            if not is_canonical_zillow_closing(
                tx_scope, report_year=report_year, current_year=date.today().year,
            ) or classify_lead(transaction.lead_source)["source_family"] != "Zillow":
                continue
            summary["eligible"] += 1
            tx = {
                "id": transaction.id,
            ...
            zillow_match = match_zillow_transaction(tx, zillow_rows)
            zillow_row = zillow_match["row"] if zillow_match else None
            candidate_people = dict(people)
            for query in _name_queries(transaction.client_name or "", zillow_row.get("Contact Name") if zillow_row else None):
                for person in (_get_json(http, f"{FUB_BASE}/people", {"q": query, "limit": 20}).get("people") or []):
                    candidate_people[str(person["id"])] = person
            match = choose_fub_person(tx, zillow_row, candidate_people.values())
            if not match:
            ...
            if apply:
                ...
                db.session.add(AuditLog(
                    ...
                    note=f"{match['method']}:{match['confidence']}",
                ))
        if apply:
            db.session.commit()
        else:
...
def main():
    ...
    print(json.dumps(run(
        ...
        since_days=args.since_days, report_year=args.year,
    ), sort_keys=True))
